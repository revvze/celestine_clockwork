	...
	for i = 2, #convTable do
		...
		timer.Create("ConvTimer_" .. uid .. "_" .. i, wait + 0.05, 1, function()
			if (IsValid(player)) then
				player:EmitSound(convTable[i], 75, 100, 0.55);
			end;
		end);
	end;
end;

Clockwork.datastream:Hook("PlayConversation", function(data)
	PLUGIN:PlayConversation(data[1], data[2]);
end);

Clockwork.datastream:Hook("GetConversationLength", function(data)
	local totalDuration = 0;
	...
	Clockwork.datastream:Start("GetConversationLength", totalDuration);
end);